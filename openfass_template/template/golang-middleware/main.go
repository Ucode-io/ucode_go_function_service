...
func NewParams(s *Settings) function.Params {
	...
	if s.enableCache {
		...
		cacheConfig.RedisHost, _ = getOrDefault("REDIS_HOST", "redis-master.redis.svc.cluster.local").(string)
		cacheConfig.RedisPort, _ = getOrDefault("REDIS_PORT", 6379).(int)
		cacheConfig.RedisUsername, _ = getOrDefault("REDIS_USERNAME", "").(string)
		cacheConfig.RedisPassword, _ = getOrDefault("REDIS_PASSWORD", "z17WuGu8dj").(string)
